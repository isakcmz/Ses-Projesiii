...
def list_data():
    # Yeni form ekranı (Toplevel) oluştur
    list_window = tk.Toplevel(app)
    list_window.title("Veritabanı Kayıtları")
    list_window.geometry("600x400")

    # Treeview oluştur (verileri tablo şeklinde göstermek için)
    tree = ttk.Treeview(list_window, columns=("ID", "Sahip", "Ses Dosyası"), show="headings")
    tree.heading("ID", text="ID")
    tree.heading("Sahip", text="Sahip")
    tree.heading("Ses Dosyası", text="Ses Dosyası")

    tree.column("ID", width=50)
    tree.column("Sahip", width=200)
    tree.column("Ses Dosyası", width=300)
...
# Ana çerçeve
main_frame = tk.Frame(app, bg="blue")
main_frame.pack(pady=20, padx=20, fill=tk.BOTH, expand=True)

# Üst kısmı oluşturan çerçeveleri yerleştir
main_frame.grid_rowconfigure(0, weight=55)  # Üst kısmın %60'ı
main_frame.grid_rowconfigure(1, weight=45)  # Alt kısmın %40'ı
